...
export function useVoiceAgent({
    ...
    silenceGracePeriodMs = 1200,
    noiseCalibrationMs = 900,
}: UseVoiceAgentProps) {
    ...
    const isRunCancelled = useCallback((runId: number) => {
    ...
    // VAD Parameters
    const SILENCE_THRESHOLD = silenceThresholdDb; // dB
    const SILENCE_DURATION = silenceDurationMs; // ms
    const SILENCE_GRACE_PERIOD = silenceGracePeriodMs; // ms
    const NOISE_CALIBRATION_MS = noiseCalibrationMs; // ms
    ...
    const playNextInQueue = useCallback(() => {
        ...
        if (audioQueueRef.current.length === 0) {
            ...
            if (!isPipelineActiveRef.current) {
                isPlayingRef.current = false;
                setAgentState("idle");
                if (isContinuous && !stopRequestedRef.current) {
    ...
    const enqueueAudio = useCallback((base64Audio: string, runId: number) => {
    ...
    }, [isRunCancelled, playNextInQueue]);
    ...
    const speakSentence = useCallback((sentence: string, runId = activeRunIdRef.current): Promise<void> => {
    ...
    }, [enqueueAudio, isRunCancelled, voiceLanguageCode, voicePace, voiceSpeaker]);
    ...
    const speakAssistantMessage = useCallback(async (message: string, addToHistory = true) => {
        ...
        await speakSentence(message, runId);
        if (isRunCancelled(runId)) {
            return;
        }
        await ttsChainRef.current;
    }, [isRunCancelled, speakSentence, stopCurrentAudio]);
        ...
        try {
            ...
            if (useCloudLLM && (llmProvider === "groq" || llmProvider === "xai")) {
                ...
                for (const chunk of allChunks) {
                    ...
                    if (cleanChunk) await speakSentence(cleanChunk, runId);
                }
    ...
    const startListening = useCallback(async () => {
        try {
            ...
            const checkSilence = () => {
                ...
                if (Date.now() - recordingStartedAt < NOISE_CALIBRATION_MS) {
                    if (Number.isFinite(db)) {
                        noiseDbSum += db;
                        noiseDbSamples += 1;
                    }
                } else if (noiseDbSamples > 0) {
                    const noiseFloorDb = noiseDbSum / noiseDbSamples;
                    adaptiveSilenceThreshold = Math.max(-32, Math.min(SILENCE_THRESHOLD, noiseFloorDb + 6));
                    noiseDbSamples = 0;
                }

                if (Date.now() - recordingStartedAt < SILENCE_GRACE_PERIOD) {
    ...
    const stopAgent = useCallback(() => {
        ...
        setAgentState("idle");
        setErrorMsg(null);
    }, [businessName, businessSheetId, businessSheetTabName, disableSideEffects, stopCurrentAudio, stopMic]);
